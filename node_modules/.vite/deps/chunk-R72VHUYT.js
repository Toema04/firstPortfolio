...
var Vector = class extends Vector3d {
  constructor(xOrCoords, y) {
    super(xOrCoords, y, origin2.z);
  }
  static get origin() {
    return Vector.create(origin2.x, origin2.y);
  }
  static clone(source) {
...
function paintBase(context, dimension, baseColor) {
  ...
  context.fillRect(origin3.x, origin3.y, dimension.width, dimension.height);
}
function paintImage(context, dimension, image, opacity) {
  ...
  context.drawImage(image, origin3.x, origin3.y, dimension.width, dimension.height);
  context.globalAlpha = 1;
}
function clear(context, dimension) {
  context.clearRect(origin3.x, origin3.y, dimension.width, dimension.height);
}
...
var Particle = class {
  constructor(engine, id, container, position, overrideOptions, group) {
    this.container = container;
    this._calcPosition = (container2, position2, zIndex, tryCount = defaultRetryCount) => {
      for (const [, plugin] of container2.plugins) {
    ...
    this._initPosition = (position2) => {
      ...
      this.position = this._calcPosition(container2, position2, clamp(zIndexValue, minZ, container2.zLayers));
...
var Circle = class extends Range {
  ...
  intersects(range) {
    ...
    if (range instanceof Circle) {
      const rSum = r + range.radius, dist = Math.sqrt(distPos.x ** squareExp3 + distPos.y ** squareExp3);
      return rSum > dist;
    } else if (range instanceof Rectangle) {
      const { width, height } = range.size, edges = Math.pow(distPos.x - width, squareExp3) + Math.pow(distPos.y - height, squareExp3);
  ...
  constructor(rectangle, capacity) {
    ...
    this._subdivide = () => {
      ...
      for (let i = 0; i < subdivideCount; i++) {
        ...
        this._subs.push(new QuadTree(new Rectangle(x + width * half2 * fixedIndex, y + height * half2 * (Math.round(i * half2) - fixedIndex), width * half2, height * half2), capacity2));
      }
      this._divided = true;
    };
    this._points = [];
...
var Particles = class {
  ...
  async update(delta) {
    ...
    for (const [, plugin] of container.plugins) {
      await ((_a = plugin.update) == null ? void 0 : _a.call(plugin, delta));
    }
...
//# sourceMappingURL=chunk-R72VHUYT.js.map

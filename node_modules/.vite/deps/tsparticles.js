...
var EmitterInstance = class {
  constructor(engine, emitters, container, options, position) {
    ...
    this._setColorAnimation = (animation, initValue, maxValue) => {
      ...
      const colorOffset = randomInRange(animation.offset), delay = getRangeValue(this.options.rate.delay), emitFactor = delay * millisecondsToSeconds / container2.retina.reduceFactor, defaultColorSpeed = 0, colorFactor = 3.6, colorSpeed = getRangeValue(animation.speed ?? defaultColorSpeed);
      return (initValue + colorSpeed * container2.fpsLimit / emitFactor + colorOffset * colorFactor) % maxValue;
    };
    this._engine = engine;
...
var BaseMover = class {
  constructor() {
    this._initSpin = (particle) => {
      ...
      particle.spin = {
        center: spinCenter,
        direction: particle.velocity.x >= minVelocity7 ? "clockwise" : "counter-clockwise",
...
function bounceHorizontal(data) {
  ...
  let bounced = false;
  if (data.direction === "right" && data.bounds.right >= data.canvasSize.width && velocity > minVelocity3 || data.direction === "left" && data.bounds.left <= boundsMin && velocity < minVelocity3) {
...
function bounceVertical(data) {
  ...
  let bounced = false;
  if (data.direction === "bottom" && data.bounds.bottom >= data.canvasSize.height && velocity > minVelocity3 || data.direction === "top" && data.bounds.top <= boundsMin && velocity < minVelocity3) {
...
var Bouncer = class extends ExternalInteractorBase {
  constructor(container) {
    ...
    this._processMouseBounce = () => {
      const container2 = this.container, pxRatio = container2.retina.pixelRatio, tolerance = toleranceFactor * pxRatio, mousePos = container2.interactivity.mouse.position, radius = container2.retina.bounceModeDistance;
    ...
    this._singleSelectorBounce = (selector, div) => {
      ...
      query.forEach((item) => {
        ...
        }, radius = elem.offsetWidth * half5 * pxRatio, tolerance = toleranceFactor * pxRatio, area = div.type === "circle" ? new Circle(pos.x, pos.y, radius + tolerance) : new Rectangle(elem.offsetLeft * pxRatio - tolerance, elem.offsetTop * pxRatio - tolerance, elem.offsetWidth * pxRatio + tolerance * double6, elem.offsetHeight * pxRatio + tolerance * double6);
  ...
  constructor(container) {
    ...
    this._singleSelectorHover = (delta, selector, div) => {
      ...
      selectors.forEach((item) => {
        ...
        }, repulseRadius = elem.offsetWidth * half6 * pxRatio, area = div.type === "circle" ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), query = container2.particles.quadTree.query(area, (p) => this.isEnabled(p));
        for (const particle of query) {
  ...
  constructor(engine, container) {
    ...
    this._singleSelectorRepulse = (selector, div) => {
      ...
      query.forEach((item) => {
        ...
        }, repulseRadius = elem.offsetWidth * half7 * pxRatio, area = div.type === "circle" ? new Circle(pos.x, pos.y, repulseRadius) : new Rectangle(elem.offsetLeft * pxRatio, elem.offsetTop * pxRatio, elem.offsetWidth * pxRatio, elem.offsetHeight * pxRatio), divs = repulse.divs, divRepulse = divMode(divs, elem);
  ...
  draw(data) {
    ...
    if (image.gif && image.gifData) {
      ...
      if (particle.gifLoopCount === void 0) {
        particle.gifLoopCount = image.gifLoopCount ?? defaultLoopCount2;
      }
      ...
      if (particle.gifTime > frame.delayTime) {
        ...
        if (++frameIndex >= image.gifData.frames.length) {
          if (--particle.gifLoopCount <= defaultLoopCount2) {
            return;
          }
...
var LifeUpdater = class {
  ...
  init(particle) {
    ...
    particle.life = {
      delay: container.retina.reduceFactor ? getRangeValue(lifeOptions.delay.value) * (lifeOptions.delay.sync ? identity3 : getRandom()) / container.retina.reduceFactor * millisecondsToSeconds : noTime,
      delayTime: noTime,
      duration: container.retina.reduceFactor ? getRangeValue(lifeOptions.duration.value) * (lifeOptions.duration.sync ? identity3 : getRandom()) / container.retina.reduceFactor * millisecondsToSeconds : noTime,
      time: noTime,
...
var Linker = class extends ParticlesInteractorBase {
  ...
  async interact(p1) {
    ...
    const linkOpt1 = p1.options.links, optOpacity = linkOpt1.opacity, optDistance = p1.retina.linksDistance ?? minDistance6, warp = linkOpt1.warp, range = warp ? new CircleWarp(pos1.x, pos1.y, optDistance, canvasSize) : new Circle(pos1.x, pos1.y, optDistance), query = container.particles.quadTree.query(range);
    for (const p2 of query) {
...
async function loadTextShape(engine, refresh = true) {
  await engine.addShape(validTypes2, new TextDrawer(), refresh);
}
